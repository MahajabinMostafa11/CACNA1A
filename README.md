...
A knowledge base tracking CACNA1A-related neurological disorders — episodic ataxia type 2, spinocerebellar ataxia type 6, developmental and epileptic encephalopathy 42, and familial hemiplegic migraine type 1.
...
<!--
  ...
  instantiation time:
    CACNA1A   Human-readable project name.
    CACNA1A      Repository slug (used for the wiki path).
    MahajabinMostafa11          GitHub owner / org (derived from origin URL).
    A knowledge base tracking CACNA1A-related neurological disorders — episodic ataxia type 2, spinocerebellar ataxia type 6, developmental and epileptic encephalopathy 42, and familial hemiplegic migraine type 1.    Project description; default text below until edited.
...
CACNA1A keeps a persistent, LLM-maintained knowledge base under `wiki/CACNA1A.wiki/` (a separate git repo), following the [llm-wiki pattern](https://github.com/tobi/llm-wiki). It is the project's durable memory: findings, decisions, experiment results, and intermediate insights belong in the wiki and accumulate over time. Three operations, **Query** (read it), **Ingest** (write to it), and **Lint** (health-check it), are codified in `wiki/CACNA1A.wiki/SCHEMA_CACNA1A.md`, in the `.claude/rules/` files, and in the `.claude/skills/` skills (`/wiki-source`, `/wiki-experiment`, `/wiki-lint`).

See also [llm-wiki.md](llm-wiki.md) in this repo for the underlying pattern.

## Quick start for collaborators

New to CACNA1A? Clone the project repo, clone the wiki as a sibling sub-repo, then seed your local Claude Code memory:

```bash
git clone https://github.com/MahajabinMostafa11/CACNA1A.git
cd CACNA1A
[ -d wiki/CACNA1A.wiki ] || \
    git clone https://github.com/MahajabinMostafa11/CACNA1A.wiki.git wiki/CACNA1A.wiki
...
The wiki at `wiki/CACNA1A.wiki/` is a separate git repo with its own history and its own remote. After any wiki edit, commit in the wiki repo (not the project repo):

```bash
git -C wiki/CACNA1A.wiki add <files>
git -C wiki/CACNA1A.wiki commit -m "..."
```

Push the wiki only when you intend to publish the changes:

```bash
git -C wiki/CACNA1A.wiki push origin master
```
