...
Follow the Ingest procedure defined in `wiki/CACNA1A.wiki/SCHEMA_CACNA1A.md`. The steps below are pointers, not a substitute for SCHEMA:
...
7. Update `index_CACNA1A.md` with one-line descriptions of new pages in the right category.
8. Append a `## [YYYY-MM-DD] update | Experiment name` entry to `log_CACNA1A.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the run. See "Log Entry Attribution" in `SCHEMA_CACNA1A.md`.
